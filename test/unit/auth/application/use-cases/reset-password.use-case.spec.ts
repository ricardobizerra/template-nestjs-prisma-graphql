...
describe('ResetPasswordUseCase', () => {
  const resetTokenRepository = {
    findValidTokensWithUser: vi.fn(),
    deleteById: vi.fn(),
  } as any;
  const passwordHasher = {
    compare: vi.fn(),
    hash: vi.fn().mockResolvedValue('new-h'),
  } as any;
  ...
  it('throws when token does not match', async () => {
    resetTokenRepository.findValidTokensWithUser.mockResolvedValue([
      { id: 't1', tokenHash: 'h1', userId: '1', user: { id: '1' } },
    ]);
    passwordHasher.compare.mockResolvedValue(false);
    await expect(useCase.execute('plain', 'new')).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('updates password and deletes token on match', async () => {
    ...
    expect(userRepository.updatePasswordAndRevokeSessions).toHaveBeenCalledWith(
      '1',
      'new-h',
    );
